...
@pytest.mark.asyncio
@patch("airweave.platform.destinations.qdrant.QdrantDestination")
async def test_count_qdrant_for_collection_success(
    mock_qdrant_class, admin_sync_service, mock_ctx, sample_syncs
):
    """Test Qdrant counting for a collection."""
    collection_id = uuid4()
    
    # Mock Qdrant destination and client
    ...
    mock_qdrant.client.count = AsyncMock(return_value=mock_count_result)
    
    mock_qdrant_class.create = AsyncMock(return_value=mock_qdrant)
    
    count_map = await admin_sync_service._count_qdrant_for_collection(
        collection_id=collection_id,
        syncs=sample_syncs,
        ctx=mock_ctx,
    )
    
    # Verify Qdrant destination was created
    mock_qdrant_class.create.assert_called_once()
    
    # Verify counts were fetched for all syncs
    assert len(count_map) == 3
    assert all(count == 100 for count in count_map.values())


@pytest.mark.asyncio
@patch("airweave.platform.destinations.qdrant.QdrantDestination")
async def test_count_qdrant_for_collection_creation_failure(
    mock_qdrant_class, admin_sync_service, mock_ctx, sample_syncs
):
    """Test Qdrant counting when destination creation fails."""
    collection_id = uuid4()
    
    # Mock creation failure
    mock_qdrant_class.create = AsyncMock(side_effect=Exception("Connection failed"))
...
@pytest.mark.asyncio
@patch("airweave.platform.destinations.qdrant.QdrantDestination")
async def test_count_qdrant_for_collection_individual_count_failure(
    mock_qdrant_class, admin_sync_service, mock_ctx, sample_syncs
):
    """Test Qdrant counting when individual counts fail."""
    collection_id = uuid4()
    
    # Mock Qdrant destination
    mock_qdrant = AsyncMock()
    mock_qdrant.collection_name = "test_collection"
    mock_qdrant.client = AsyncMock()
    mock_qdrant.client.count = AsyncMock(side_effect=Exception("Query failed"))
